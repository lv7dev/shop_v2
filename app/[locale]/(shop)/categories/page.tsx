...
export default async function CategoriesPage({ params }: Props) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold">{t("category.shopByCategory")}</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          {t("category.browseCount", { count: categories.length })}
        </p>
      </div>
      ...
      {categories.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-4 py-24 text-center">
          ...
          <div>
            <h2 className="text-lg font-semibold">{t("category.noCategories")}</h2>
                  ...
                  <CardContent className="flex flex-col gap-4 p-6">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <Badge variant="secondary">
                        {t("category.productCount", { count: totalProducts })}
                      </Badge>
                      {category.children && category.children.length > 0 && (
                        <Badge variant="outline">
                          {t("category.subcategoryCount", { count: category.children.length })}
                        </Badge>
                      )}
                    </div>
